...
import pandas as pd
import numpy as np
from helpers import parse_output_dir, parse_input_dir, create_digest_re
...
parser.add_argument('--specificity', type=float, default=1.0,
                    help='1 - probability that cleavage occurs after any residue at random. [default: 1.0]')
...
parser.add_argument('--repeats', type=int, default=1,
                    help='Number of times to repeat digestion. Note: only makes sense if '
                         'efficiency/specificity/resolution is set, otherwise fingerprints will end up the same each '
                         'time. [default: 1]')
parser.add_argument('--enzyme', type=str, default='trypsin',
                    help='Define which enzyme to use for digestion [default: trypsin]')
...
for fai, fa_fn in enumerate(fasta_list):
    for nr in range(args.repeats):
        ...
        #     cleave_idx = np.concatenate(([0],
        #       np.argwhere(np.random.random(len(seq)-2) > args.specificity).squeeze(-1) + 1, [len(seq)]))
